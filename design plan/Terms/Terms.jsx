...
export default function Terms() {
  ...
  useEffect(() => {
    let cancelled = false;
    termsAPI.getCurrent()
      .then(r => { if (!cancelled) setCurrentVersion(r.data.terms); })
      .catch(() => {});
  ...
  return (
    <div style={styles.container}>
      <div style={styles.content}>
        ...
        {currentVersion && (
          <div style={{ padding: '.75rem 1rem', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 6, marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '.5rem' }}>
            <div>
              <strong>Versiune curentă: {currentVersion.version}</strong>
              <span style={{ marginLeft: '.5rem', color: '#666', fontSize: 13 }}>
                În vigoare din {new Date(currentVersion.effective_date).toLocaleDateString('ro-RO', { day: 'numeric', month: 'long', year: 'numeric' })}
              </span>
            </div>
          </div>
        )}
