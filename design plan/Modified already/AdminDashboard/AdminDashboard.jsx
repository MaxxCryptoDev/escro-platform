...
export default function AdminDashboard() {
  ...
  const tabs = [
    ...
    { id: 'users',      label: 'Utilizatori',   icon: 'users',       count: stats.pending_kyc || pendingUsers.length || undefined },
    { id: 'projects-tasks', label: 'Proiecte & Taskuri', icon: 'folder', count: ((pendingProjects.length || 0) + (pendingTasks.length || 0)) || undefined },
    { id: 'applications', label: 'Aplicații', icon: 'send', count: pendingApplications.length || undefined },
    { id: 'financiar',  label: 'Financiar',     icon: 'trending-up' },
    { id: 'contracte',  label: 'Contracte',     icon: 'file-text' },
    { id: 'dispute',    label: 'Dispute',       icon: 'alert-triangle', count: stats.pending_disputes || pendingDisputes.length || undefined },
    { id: 'calls',      label: 'Apeluri identitate', icon: 'phone',   count: pendingCalls.length || undefined },
    { id: 'trust',      label: 'Trust',         icon: 'shield' },
    { id: 'referral',   label: 'Referral',      icon: 'gift' },
    { id: 'audit',      label: 'Audit Log',     icon: 'list' },
    { id: 'terms',      label: 'T&C',           icon: 'file-text' },
  ];
  ...
  return (
    <div className="escro-page fade-up">
      <div className="page-head" style={{ marginBottom: '1.5rem' }}>
        <div>
          <div className="h-eyebrow">Dashboard · Admin</div>
          <h1 className="h-title" style={{ fontSize: 28 }}>Administrare <em>platformă</em>.</h1>
          <p className="h-sub">{stats.total_users || allUsers.length || '…'} utilizatori · {stats.total_projects || allProjects.length || '…'} proiecte · {fmtRON(stats.total_revenue || 0)} comision</p>
        </div>
      </div>

      {/* Stats */}
      <div className="stat-grid section">
        <Stat label="Total utilizatori" value={allUsers.length} suffix="" />
        <Stat label="Experți verificați" value={stats.verified_experts || 0} suffix="" />
        <Stat label="Escrow blocat" value={stats.escrow_held || 0} suffix=" RON" format={v => fmtRON(v).replace(' RON','')} />
        <Stat label="Comision total" value={stats.total_revenue || 0} suffix=" RON" format={v => fmtRON(v).replace(' RON','')} />
      </div>
        ...
        <div className="row" style={{ gap: 0, flexWrap: 'wrap' }}>
          {tabs.map(t => (
            <div key={t.id} onClick={() => setActiveTab(t.id)} style={{
              ...
              {t.label}
              {t.count != null && (
                <span style={{ fontSize: 10, background: 'var(--danger)', color: '#fff', padding: '1px 5px', borderRadius: 99, fontWeight: 700 }}>{t.count}</span>
              )}
            </div>
          ))}
        </div>
      ...
      {!loading && !tabLoading && activeTab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1.5rem', alignItems: 'start' }}>
          <div className="col" style={{ gap: '1.5rem' }}>
            {/* Pending actions */}
            {(pendingUsers.length > 0 || pendingProjects.length > 0 || pendingDisputes.length > 0 || pendingCalls.length > 0) && (
              <div className="card" style={{ borderColor: 'var(--warning-border)' }}>
                <div className="card-head">
                  <div className="row" style={{ gap: '.5rem', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--warning)' }}>Necesită atenție</span>
                  </div>
                </div>
                <div className="card-body" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px,1fr))', gap: '.75rem' }}>
                  {[
                    { label: 'KYC în așteptare', count: pendingUsers.length, tab: 'users', icon: 'users' },
                    { label: 'Proiecte de aprobat', count: pendingProjects.length, tab: 'projects-tasks', icon: 'folder' },
                    { label: 'Dispute deschise', count: pendingDisputes.length, tab: 'dispute', icon: 'alert-triangle' },
                    { label: 'Apeluri identitate', count: pendingCalls.length, tab: 'calls', icon: 'phone' },
                    { label: 'Task-uri pending', count: pendingTasks.length, tab: 'projects-tasks', icon: 'kanban' },
                  ].filter(x => x.count > 0).map(x => (
                    <div key={x.tab} onClick={() => setActiveTab(x.tab)} style={{
                      padding: '.75rem', background: 'var(--bg-2)', borderRadius: 8,
                    ...
                    >
                      <Icon name={x.icon} size={16} style={{ color: 'var(--warning)', flexShrink: 0 }} />
                      <div>
                        <div className="mono" style={{ fontWeight: 700, fontSize: 18, color: 'var(--fg-0)' }}>{x.count}</div>
                        <div style={{ fontSize: 11, color: 'var(--fg-3)' }}>{x.label}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {financiar?.summary && (
              <div className="card">
                <div className="card-head">
                  <div className="card-title">Financiar</div>
                  <button className="btn btn-ghost btn-sm" onClick={() => setActiveTab('financiar')}>
                    Detalii <Icon name="arrow-right" size={12} />
                  </button>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1px', background: 'var(--border-1)', borderRadius: 8, overflow: 'hidden' }}>
                  {[
                    { label: 'Comision câștigat', value: financiar.summary.total_commission, color: 'var(--success)' },
                    { label: 'Plătit experților', value: financiar.summary.total_released_experts, color: 'var(--accent)' },
                    { label: 'Escrow blocat', value: financiar.summary.currently_held, color: 'var(--warning)' },
                  ].map(x => (
                    <div key={x.label} style={{ padding: '1rem', background: 'var(--bg-1)', textAlign: 'center' }}>
                      <div className="mono" style={{ fontSize: 17, fontWeight: 700, color: x.color }}>{fmtRON(parseFloat(x.value) || 0)}</div>
                      <div style={{ fontSize: 11, color: 'var(--fg-3)', marginTop: 3 }}>{x.label}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="card">
              <div className="card-head">
                <div className="card-title">Utilizatori recenți</div>
                <button className="btn btn-ghost btn-sm" onClick={() => setActiveTab('users')}>
                  Toți <Icon name="arrow-right" size={12} />
                </button>
              </div>
              <table className="tbl">
                <thead><tr><th>Utilizator</th><th>Rol</th><th>Stripe KYC</th><th>Înregistrat</th></tr></thead>
                <tbody>
                  {allUsers.slice(0, 7).map(u => (
                    <tr key={u.id}>
                      ...
                      <td>
                        <span className={`badge ${u.kyc_status === 'verified' ? 'badge-green' : 'badge-amber'}`}>
            ...
            <div className="card">
              <div className="card-head">
                <div className="card-title">Proiecte recente</div>
                <button className="btn btn-ghost btn-sm" onClick={() => setActiveTab('projects')}>
                  Toate <Icon name="arrow-right" size={12} />
                </button>
              </div>
              <table className="tbl">
                <thead><tr><th>Proiect</th><th>Status</th><th>Buget</th><th>Data</th></tr></thead>
                <tbody>
                  {allProjects.slice(0, 6).map(p => (
                    <tr key={p.id} onClick={() => navigate(`/project/${p.id}`)} style={{ cursor: 'pointer' }}>
                      <td style={{ fontWeight: 600 }}>{p.title}</td>
                      <td><StatusBadge status={p.status} /></td>
                      <td className="mono">{fmtRON(p.budget_ron || p.budget || 0)}</td>
          ...
          <AdminActivityPanel
            ...
            onLoadMore={async () => {
              ...
              try {
                const res = await adminAPI.getAdminActivity(nextPage);
                const d = res.data;
                setActivity(prev => [...prev, ...(d.activity || [])]);
                setActivityPage(nextPage);
                setActivityHasMore(nextPage < (d.pagination?.pages || 1));
                  ...
                  return (
                    <tr key={p.id} onClick={() => navigate(`/project/${p.id}`)} style={{ cursor: 'pointer', background: hasDispute ? 'var(--danger-bg)' : undefined }}>
                      <td>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--fg-3)' }}>{p.service_type || '—'}</div>
                      </td>
              ...
              <div className="col" style={{ gap:'1rem' }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
                  <div className="card" style={{ padding:'.75rem' }}>
                    ...
                    <div style={{ fontWeight:600 }}>{detailModal.service_type || detailModal.assignment_type || '—'}</div>
                  </div>
                  <div className="card" style={{ padding:'.75rem' }}>
...
function AdminProjectRow({ p, navigate, isSubtask, isParent, handleOpenDetail, handleApproveProject, handleOpenCreateTaskModal, handleOpenAssignModal, handleDeleteProject, handleRefund, handleOpenEdit, handleRemoveExpert, handleOpenHistory, handleRejectProject, loadingActions }) {
  ...
  return (
    <tr style={isSubtask ? { background: 'var(--bg-0)' } : isParent ? { background: 'var(--bg-1)' } : {}}>
      ...
      <td>
        {!isPm && !isSubtask && <span className="tag" style={{ fontSize: 10 }}>{p.service_type || 'matching'}</span>}
